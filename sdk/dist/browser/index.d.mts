...
import { RegistryTypes, Signer, Codec, Registry, ISubmittableResult } from '@polkadot/types/types';
import { ApiPromise, SubmittableResult } from '@polkadot/api';
import * as _polkadot_api_types from '@polkadot/api/types';
import { Signer as Signer$1, SubmittableExtrinsic, ApiTypes, DecorateMethod, ApiOptions } from '@polkadot/api/types';
import { Bytes, Compact, u64 } from '@polkadot/types-codec';
import { ContractInstantiateResult, AccountId, Hash } from '@polkadot/types/interfaces';
import { IEnum } from '@polkadot/types-codec/types';
import { Abi } from '@polkadot/api-contract/Abi';
import { BN } from '@polkadot/util';
import { KeyringPair } from '@polkadot/keyring/types';
import { KeypairType } from '@polkadot/util-crypto/types';
import { MessageMeta, ContractCallResult, MapMessageTx, MapConstructorExec } from '@polkadot/api-contract/base/types';
import * as _polkadot_api_base_types from '@polkadot/api-base/types';
import { U8aFixed, Option, Map, Vec, Enum, Text } from '@polkadot/types';
...
type AbiLike = string | Record<string, unknown> | Abi;
type WasmLike = Uint8Array | string | Buffer | null | undefined;
...
type QueryFn = (encodedQuery: string, certificateData: CertificateData) => Promise<string>;
type SidevmQuery<T = string> = (bytes: Bytes, certificateData: CertificateData) => Promise<T>;
type CommandFn = (params: {
    contractId: string;
    payload: string;
    deposit: BN;
...
interface ContractInkQuery$1<ApiType extends ApiTypes> extends MessageMeta {
    (origin: string | AccountId | Uint8Array, ...params: unknown[]): ContractCallResult<ApiType, ContractCallOutcome>;
}
interface MapMessageInkQuery$1<ApiType extends ApiTypes> {
    [message: string]: ContractInkQuery$1<ApiType>;
}
interface PinkContractQueryOptions {
    cert: CertificateData;
}
declare function pinkQuery(api: ApiPromise, pruntimeApi: pruntime_rpc.PhactoryAPI, pk: Uint8Array, queryAgreementKey: Uint8Array, encodedQuery: string, { certificate, pubkey, secret }: CertificateData): Promise<`0x${string}`>;
declare class PinkContractPromise {
    #private;
    readonly abi: Abi;
    ...
    get query(): MapMessageInkQuery$1<'promise'>;
    get tx(): MapMessageTx<'promise'>;
}

declare class UnexpectedEndpointError extends Error {
}
...
declare class OnChainRegistry {
    ...
    pruntimeURL: string | undefined;
    constructor(api: ApiPromise);
    getContractKey(contractId: AccountId | string): Promise<string | undefined>;
    getContractKeyOrFail(contractId: string): Promise<string>;
    isReady(): boolean;
    get phactory(): pruntime_rpc.PhactoryAPI;
    get gasPrice(): BN;
    /**
     * Static factory method returns a ready to use PhatRegistry object.
     */
    static create(api: ApiPromise, options?: CreateOptions): Promise<OnChainRegistry>;
    getClusters(clusterId?: string): Promise<ClusterInfo | (string | ClusterInfo)[][] | null>;
    getEndpints(workerId?: U8aFixed | string): Promise<Option<VersionedEndpoints> | (string | Option<VersionedEndpoints>)[][]>;
    /**
      ...
      */
    connect(clusterId?: string | null, workerId?: string | null, pruntimeURL?: string | null, systemContractId?: string | AccountId, skipCheck?: boolean): Promise<void>;
    get systemContract(): PinkContractPromise | undefined;
    getClusterBalance(pair: KeyringPair, address: string | AccountId): Promise<{
        total: BN;
        free: BN;
    }>;
    transferToCluster(address: string | AccountId, amount: number | string | BN): _polkadot_api_base_types.SubmittableExtrinsic<"promise", _polkadot_types_types.ISubmittableResult>;
}
...
declare class PinkBlueprintSubmittableResult extends SubmittableResult {
    #private;
    ...
    waitFinalized(timeout?: number): Promise<void>;
    get contract(): PinkContractPromise | undefined;
}
declare class PinkBlueprintPromise {
    #private;
    readonly abi: Abi;
    readonly api: ApiBase<'promise'>;
    readonly phatRegistry: OnChainRegistry;
    ...
    get tx(): MapConstructorExec<'promise'>;
    get query(): MapMessageInkQuery<'promise'>;
}

declare class InkCodeSubmittableResult extends SubmittableResult {
    #private;
    readonly registry: OnChainRegistry;
    readonly abi: Abi;
    readonly blueprint?: PinkBlueprintPromise;
    constructor(result: ISubmittableResult, abi: Abi, registry: OnChainRegistry);
    waitFinalized(pair: KeyringPair, cert: CertificateData, timeout?: number): Promise<void>;
}
declare class PinkCodePromise {
    #private;
    readonly abi: Abi;
    readonly api: ApiBase<'promise'>;
    readonly phatRegistry: OnChainRegistry;
    ...
    get tx(): MapConstructorExec<'promise'>;
    upload(): _polkadot_api_types.SubmittableExtrinsic<"promise", ISubmittableResult>;
}

declare class PinkLoggerContractPromise extends PinkContractPromise {
    #private;
    static create(api: ApiPromise, registry: OnChainRegistry, systemContract: PinkContractPromise, pair?: KeyringPair): Promise<PinkLoggerContractPromise>;
    constructor(api: ApiPromise, registry: OnChainRegistry, abi: any, contractId: string, contractKey: string, pair?: KeyringPair);
    getLog(contractId: AccountId | string, from?: number, counts?: number): Promise<any>;
}

declare function options(options?: ApiOptions): ApiOptions;

declare const PhactoryAPI: typeof pruntime_rpc.PhactoryAPI;
declare const abis: {
    system: {
        source: {
            hash: string;
...
export { CertificateData, ClusterInfo$1 as ClusterInfo, CommandFn, ContractExecResultWeightV2, ContractInfo, ContractInkQuery$1 as ContractInkQuery, CreateFnOptions, CreateFnResult, InkCodeSubmittableResult, MapMessageInkQuery$1 as MapMessageInkQuery, OnChainRegistry, PhactoryAPI, PhalaInstance, PinkBlueprintPromise, PinkBlueprintSubmittableResult, PinkCodePromise, PinkContractPromise, PinkContractQueryOptions, PinkLoggerContractPromise, QueryFn, SidevmQuery, UnexpectedEndpointError, abis, create, createPruntimeApi, options, pinkQuery, randomHex, ready, signCertificate, types };
